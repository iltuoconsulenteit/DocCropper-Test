...
Run `./install_DocCropper.sh` to clone or update the project in `~/Scrivania/DocCropper`. The script checks for `git`, `python3`, and `pip3`. It performs `git pull --rebase --autostash` so local edits (like custom `settings.json`) are stashed and restored automatically. After updating it merges any new keys from the repository version of `settings.json` into your local copy so your preferences are preserved while new options appear.
